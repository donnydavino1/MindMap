...
TREE_COLUMNS = ["Task", "Category", "Subcategory", "Tags", "Urgency", "Importance",
...
class TaskManager:
    ...
    def _build_header(self):
        ...
        e = ctk.CTkEntry(q, textvariable=self.quick_var,
                         placeholder_text="Quick add:  fix XRD ~tomorrow !1 *2 #urgent @PLD")
        e.pack(side="left", fill="x", expand=True, padx=(8, 6), pady=8)
        e.bind("<Return>", lambda ev: self._quick_add())
        self.qcat_var = tk.StringVar(value="Other")
        self.qcat_box = ctk.CTkComboBox(q, width=150, variable=self.qcat_var,
                                        values=self._cat_values(),
                                        command=lambda v: self._on_pick_category(v, self.qcat_var))
        self.qcat_box.pack(side="left", padx=4)
        self.qurg_var = tk.StringVar(value="3")
        ctk.CTkComboBox(q, values=["1", "2", "3", "4", "5"], variable=self.qurg_var,
                        width=64).pack(side="left", padx=4)
        ctk.CTkButton(q, text="Add Task", width=96, command=self._quick_add).pack(side="left", padx=(4, 8))
    ...
    def _build_list_tab(self, tab):
        ...
        self.tree = ttk.Treeview(tf, columns=TREE_COLUMNS, show="headings", selectmode="extended")
        for c in TREE_COLUMNS:
            self.tree.heading(c, text=c, command=lambda cc=c: self._sort_column(cc))
            anchor = "w" if c in ("Task", "Tags", "Category", "Subcategory") else "center"
            self.tree.column(c, width=TREE_WIDTHS[c], anchor=anchor,
                             stretch=(c in ("Task", "Tags")))
    ...
    def _quick_add(self):
        ...
        row = self._new_task(
            ...
            Importance=parsed["Importance"] if parsed["Importance"] else 3,
    ...
    def refresh_list(self):
        ...
        for _, row in view.iterrows():
            self.tree.insert("", "end", iid=str(int(row["ID"])),
    ...
    def _open_edit_dialog(self, _id, row):
        ...
        ctk.CTkLabel(form, text="Subtasks (one per line, '[x]' = done)", anchor="w"
                     ).pack(fill="x", pady=(8, 2))
        subbox = ctk.CTkTextbox(form, height=90)
        subbox.pack(fill="x")
        subbox.insert("1.0", str(row["Subtasks"]))
        ...
        def open_link():
            ...
            try:
                ...
                messagebox.showerror("Open link", str(ex), parent=win)
        ...
        def save_changes():
            ...
            self.df.loc[mask, "Subtasks"] = subbox.get("1.0", "end").strip()
